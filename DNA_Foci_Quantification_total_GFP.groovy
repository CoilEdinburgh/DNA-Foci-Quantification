...
class DNA_Foci_Quantification implements Command {
   ...
   // private LegacyService legacyService
    ...
    void run() {
        ...
        def newDirectory = directory + "Output_DAPI_thres_Total_GFP"
        filePath = Paths.get(directory, "Output_DAPI_thres_Total_GFP").toString()
                        ...
                        if (count > 2) {
                            ...
                            for (int p = 0; p < count; p++) {
                                ...
                                try {
                                    AddToResults("", resultsLine[0], resultsLine[1], resultsLine[2], backgroundBlue, resultsLine[4], backgroundGreen,
                                            resultsLine[6] * pixelSize * pixelSize, resultsLine[7] * pixelSize * pixelSize, resultsLine[2] / backgroundBlue, resultsLine[4] / backgroundGreen, resultsLine[6]*resultsLine[5])
                                } catch (IOException e) {
                                    e.printStackTrace()
                                }
                            }
                            ...
                            try {
                                AddToResults("Average_", (j + 1), count, sumIntensityBlue / count, backgroundBlue, sumIntensityGreen / count, backgroundGreen,
                                        ...
                                        sumIntensityGreen / (count * backgroundGreen),results.get(0)[6]*results.get(0)[5])
                            } catch (IOException e) {
                                e.printStackTrace()
                            }
                        }
...
//
    ...
    void NewFileResults(String name, String id) throws IOException {
        ...
        try {
            ...
            bufferedWriter.newLine()
            bufferedWriter.write("Cell,Spots,DAPI,DAPI_Background,GFP,GFP_Background,Cell_Area(um2),SpotArea(um2),DAPI_ratio,GFP_ratio,Total_GFP_Int")
    ...
    void AddToResults(String name, double cell, spot, double DAPI, double DAPIcell, double GFP, double GFPcell, double areaCell,
                      double areaSpot, double DAPI_ratio, double GFP_ratio, double totalGFP) throws IOException {
        ...
        try {
            ...
            BufferedWriter bufferedWriter = new BufferedWriter(fileWriter)
            bufferedWriter.newLine()
            bufferedWriter.write(cell + "," + spot + "," + DAPI + "," + DAPIcell + "," + GFP + "," + GFPcell + "," + areaCell + "," + areaSpot + "," + DAPI_ratio + "," + GFP_ratio+ ","+totalGFP)
            bufferedWriter.close()
        } catch (IOException ignored) {
            System.out.println(
                    "Error writing to file '" + CreateName + "'")
